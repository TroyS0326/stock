...
from db import get_failed_trades_today, get_recent_trades, insert_trade
from scanner import now_et
...
def _today_trades(symbol=None):
    day = datetime.now(ZoneInfo(config.TIMEZONE_LABEL)).date().isoformat()
    out=[]
    for t in get_recent_trades(200):
        if str(t.get('created_at','')).startswith(day):
...
def validate_trade_candidate(candidate, auto=False):
    ...
    details=candidate.get('details') or {}
    if float(details.get('spread_pct',0) or 0) > config.MAX_SPREAD_PCT: skip.append('wide_spread')
    if float(candidate.get('current_price',0)) > float(candidate.get('buy_upper',0)): skip.append('price_extended')
    if int(candidate.get('qty',0) or 0) < 1: skip.append('qty_zero')
    ...
    if auto and len(_today_trades()) >= config.MAX_AUTO_TRADES_PER_DAY: skip.append('max_auto_trades_reached')
    if symbol and (not config.ALLOW_DUPLICATE_SYMBOL_TRADES_PER_DAY) and _today_trades(symbol): skip.append('duplicate_symbol_trade_blocked')
